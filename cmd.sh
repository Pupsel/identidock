...
#Еслиe значение переменной ENV равно DEV, то запускается веб-сервер для отладки, в противном случае будет использоваться «настоящий» сервер для эксплуатации. Команда exec нужна для того, чтобы не создавать нового процесса и обеспечить получение и обработку любых сигналов (таких как SIGTERM) единственным процессом uwsgi, а не избыточным родительским процессом.
